create table if not exists public.seekmy_activities (
  ...
  location_id text not null, location_name text not null, state text not null default '',
  activity text not null default '', is_hidden_gem boolean not null default false,
...
create table if not exists public.seekmy_badges (
  firebase_uid text not null, badge_key text not null, name text not null,
  description text not null, icon text not null, progress numeric not null default 0,
  earned_at timestamptz not null default now(), primary key (firebase_uid, badge_key)
);
create table if not exists public.seekmy_user_stats (
  firebase_uid text primary key, display_name text not null default 'Explorer',
  total_km numeric(10,2) not null default 0, checkins integer not null default 0,
  states integer not null default 0, updated_at timestamptz not null default now()
);
