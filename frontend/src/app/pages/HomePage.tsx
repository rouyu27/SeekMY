...
/** Helper to resolve hero image URLs cleanly */
const getHeroPhotoUrl = (filename: string) =>
  new URL(`../../assets/hero/${filename}`, import.meta.url).toString();

const HERO_PHOTOS = [
  { src: getHeroPhotoUrl("mount-kinabalu.jpg"), alt: "Mount Kinabalu hiking trail above the clouds" },
  { src: getHeroPhotoUrl("rainforest-trail.jpg"), alt: "Misty rainforest hiking trail" },
  { src: getHeroPhotoUrl("snorkeling.jpg"), alt: "Snorkeling in clear Malaysian water" },
  { src: getHeroPhotoUrl("city-park.jpg"), alt: "City park walking path" },
  { src: getHeroPhotoUrl("park-lake.jpg"), alt: "Park lake walking path" },
  { src: getHeroPhotoUrl("aquatic-centre.jpg"), alt: "Outdoor aquatic centre swimming pool" },
];

/** Formats dynamic location/activity counts into rounded display labels (e.g. 10+, 100+) */
...
export function HomePage({
  ...
  // Hero Image Slideshow Timer
  useEffect(() => {
    if (heroPaused) return;
    const timer = window.setInterval(() => {
      setHeroIndex((current) => (current + 1) % HERO_PHOTOS.length);
    }, 4500);
    ...
    return {
      locs: locations.filter(
        (loc) =>
          ...
          loc.activity.toLowerCase().includes(q)
      ),
  ...
  // Hidden Gems & Hero Stats Calculation
  const hiddenGems = useMemo(
    () => locations.filter((loc: any) => loc.is_hidden_gem).slice(0, 3),
    [locations]
  );
  ...
  const heroStats = useMemo(() => {
    const activityCount =
      new Set(locations.map((loc) => loc.activity).filter(Boolean)).size ||
      ACTIVITY_FILTERS.filter((act) => act.id !== "all").length;
  ...
  // Hidden Gem Slideshow Timers & Guards
  useEffect(() => {
    if (hiddenGems.length <= 1) return;
    const timer = window.setInterval(() => {
      setGemIndex((current) => (current + 1) % hiddenGems.length);
    }, 4200);
    return () => window.clearInterval(timer);
  }, [hiddenGems.length]);

  useEffect(() => {
    if (gemIndex >= hiddenGems.length) setGemIndex(0);
  }, [gemIndex, hiddenGems.length]);
  ...
  function moveHero(direction: -1 | 1) {
    setHeroIndex((current) => (current + direction + HERO_PHOTOS.length) % HERO_PHOTOS.length);
  }

  function openGem() {
    if (!gem) return;
    setSelectedLocation(gem);
    setPage("location");
  }
    ...
    <div className="pt-14 min-h-screen" style={{ backgroundColor: C.cream }}>
      ...
      <section className="relative overflow-hidden" style={{ backgroundColor: C.jungle, minHeight: 310 }}>
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(135deg, rgba(27,67,50,0.72) 0%, rgba(18,52,42,0.62) 48%, rgba(10,35,24,0.74) 100%)",
          }}
        />

        {/* Subtle background particle points */}
        {[...Array(20)].map((_, i) => (
          <div
            key={i}
        ...
        <div className="relative max-w-4xl mx-auto px-6 py-14 text-center">
          <div
            className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-6 text-[11px] font-bold tracking-wide"
            style={{
          ...
          <h1 className="text-[2.8rem] md:text-5xl font-normal leading-[1.1] text-white mb-2" style={{ fontFamily: F.display }}>
            Discover Malaysia's
          </h1>
          <h1 className="text-[2.8rem] md:text-5xl font-normal leading-[1.1] mb-6" style={{ fontFamily: F.display, color: C.amber }}>
            Outdoor Adventures
          </h1>
          ...
          <div className="max-w-lg mx-auto mb-8">
            <div className="flex items-center bg-white rounded-full" style={{ boxShadow: "0 4px 24px rgba(0,0,0,0.24)", height: 46 }}>
              ...
              <button
                type="button"
                className="m-1 px-5 h-9 rounded-full text-sm font-bold text-white active:scale-95 transition-all"
          ...
          {/* Pagination Controls & Play/Pause */}
        ...
        <div className="flex gap-2 px-5 py-2.5 overflow-x-auto" style={{ scrollbarWidth: "none" }}>
          {ACTIVITY_FILTERS.map(({ id, label, icon }) => (
            <button
              ...
              className="flex items-center gap-1.5 px-4 h-9 rounded-full text-[12px] font-bold whitespace-nowrap flex-shrink-0 transition-all active:scale-95"
              style={{
                backgroundColor: activeAct === id ? C.jungle : C.muted,
                color: activeAct === id ? "#fff" : C.textSub,
                fontFamily: F.body,
              }}
            >
              {icon} {label}
      ...
      {searchResults ? (
        <div className="max-w-5xl mx-auto px-5 py-8">
          {searchResults.states.length > 0 && (
            <>
              <p className="text-xs font-bold uppercase tracking-wide mb-3" style={{ color: C.textMuted, fontFamily: F.body }}>
                States
              </p>
          ...
          {searchResults.locs.length > 0 && (
            <>
              <p className="text-xs font-bold uppercase tracking-wide mb-3" style={{ color: C.textMuted, fontFamily: F.body }}>
                Locations ({searchResults.locs.length})
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {searchResults.locs.map((loc) => (
                  <LocationCard
                    key={loc.id}
                    loc={loc}
                    bookmarked={bookmarks.includes(loc.id)}
                    onBookmark={() => onBookmark(loc.id)}
                    onView={() => {
        ...
        <div className="max-w-5xl mx-auto px-5 py-10 space-y-12">
          ...
          <div>
            <div className="flex items-center gap-2.5 mb-5">
              ...
              <h2 className="text-2xl font-normal" style={{ fontFamily: F.display, color: C.text }}>
                Hidden Gem Spotlight
              </h2>
            </div>
            <div
              ...
              <div className="p-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <h3 className="text-lg font-normal text-white mb-0.5" style={{ fontFamily: F.display }}>
                        {gem?.name || "No hidden gem selected"}
                      </h3>
                      <p className="text-xs mb-3" style={{ color: "rgba(255,255,255,0.72)", fontFamily: F.body }}>
                        {gem?.state ? `${gem.state}, Malaysia` : "Choose hidden gems in Admin Location Management"}
                      </p>
                      <p className="text-sm leading-relaxed" style={{ color: "rgba(255,255,255,0.78)", fontFamily: F.body, maxWidth: 520 }}>
                        {(gem as any)?.description || "Ask an admin to mark Firebase locations as hidden gems."}
                      </p>
                      {gem && (
                        <p className="text-xs mt-3 font-bold" style={{ color: "rgba(255,255,255,0.9)", fontFamily: F.body }}>
                          Tap to view details
                        </p>
                      )}
                    </div>
                  ...
                  <span
                    className="self-start flex-shrink-0 text-[11px] font-bold px-3 py-1 rounded-full"
                    style={{
                ...
                <div className="flex items-center gap-1.5 px-6 pb-5" onClick={(e) => e.stopPropagation()}>
                  {hiddenGems.map((item, i) => (
                    <button
                      ...
                      aria-label={`Show hidden gem ${item.name}`}
                      className="rounded-full transition-all duration-200"
                      style={{
                        width: gemIndex === i ? 22 : 7,
                        height: 7,
                        backgroundColor: gemIndex === i ? "rgba(255,255,255,0.92)" : "rgba(255,255,255,0.36)",
                      }}
                    />
                  ))}
                </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {locations
                .filter((loc) => activeAct === "all" || loc.activity === activeAct)
                .slice(0, 3)
                .map((loc) => (
                  <LocationCard
                    key={loc.id}
          ...
          {/* State Sections */}
          <div>
            <SectionHead title="Peninsular Malaysia" />
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {PENINSULAR.map((state) => (
                <StateFlagCard
                  key={state.code}
                  code={state.code}
                  name={state.name}
          ...
          <div>
            <SectionHead title="East Malaysia" />
          ...
          <div>
            <SectionHead title="Federal Territories" />
            ...
            <div className="p-8 flex flex-col sm:flex-row items-start sm:items-center gap-6">
              <div className="flex-1">
                <div
                  ...
                  style={{ backgroundColor: "rgba(255,255,255,0.10)", color: "rgba(255,255,255,0.82)", fontFamily: F.body }}
                >
                  <MessageCircle size={11} /> AI Outdoor Assistant
                </div>
                <h3 className="text-2xl font-normal text-white mb-2" style={{ fontFamily: F.display }}>
                  Ask anything about Malaysia's outdoors
                </h3>
                <p className="text-sm" style={{ color: "rgba(255,255,255,0.58)", fontFamily: F.body }}>
                  Gear tips, trail conditions, beginner guides — instant answers.
                </p>
              </div>
              <Pill variant="amber" onClick={() => setPage("ai")}>
                Open AI Guide <ArrowRight size={14} />
              </Pill>
            </div>
          </div>
        </div>
