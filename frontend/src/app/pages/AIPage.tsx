...
function FormattedAIText({text}:{text:string}) {
  ...
  return <div className="space-y-2.5">
    {lines.map((raw,index)=>{
      const line=raw.trim();
      if(!line)return <div key={index} className="h-0.5"/>;
      if(/^#{1,3}\s/.test(line))return <p key={index} className="font-bold text-[15px] text-white"><InlineMarkdown text={line.replace(/^#{1,3}\s/,"")}/></p>;
      if(/^[-*•]\s+/.test(line))return <div key={index} className="flex items-start gap-2.5 pl-0.5">
        <span className="mt-[7px] w-1.5 h-1.5 rounded-full flex-shrink-0" style={{backgroundColor:C.amber}}/>
        <p className="leading-6" style={{color:"rgba(255,255,255,0.88)"}}><InlineMarkdown text={line.replace(/^[-*•]\s+/,"")}/></p>
      </div>;
      return <p key={index} className="leading-6"><InlineMarkdown text={line}/></p>;
    })}
  </div>;
}
...
export function AIPage({ locations }: { locations: Location[] }) {
  ...
  async function send(msg?:string) {
    ...
    const localResponse = buildAIResponse(text, locations);
    try {
      const aiText = await firebaseClient.integrations.Core.InvokeLLM({
        ...
        locations: locations.slice(0,50).map(location=>({
          name: location.name,
      ...
      console.error("AI Guide request failed; using local response.", error);
      setMessages(m=>[...m,{id:Date.now()+1,from:"bot",...localResponse,time:now}]);
    } finally {
      setLoading(false);
    }
  }
              ...
              <div className={`flex flex-col gap-2 ${m.from==="user"?"items-end":"items-start"} max-w-[85%]`}>
                ...
                {m.type==="text" && m.text && (
                  <div className="px-4 py-3 text-sm leading-relaxed"
                    ...
                    {m.from==="bot"?<FormattedAIText text={m.text}/>:m.text}
                  </div>
                )}
                ...
                {m.type==="locations" && (
                  <div className="w-full max-w-sm">
                    {m.text && <div className="px-4 py-3 mb-3 text-sm rounded-2xl" style={{backgroundColor:"rgba(255,255,255,0.12)",color:"#fff",fontFamily:F.body}}><FormattedAIText text={m.text}/></div>}
