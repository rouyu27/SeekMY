...
type BadgeForm = {
  key: string; name: string; description: string; icon: string; imageUrl: string;
  metric: BadgeMetric; requirement: string; displayOrder: string;
};

const EMPTY_FORM: BadgeForm = {
  key: "", name: "", description: "", icon: "🏅", imageUrl: "",
...
function formFor(badge: ManagedBadgeDefinition): BadgeForm {
  return {
    ...
    icon: badge.icon, imageUrl: badge.imageUrl, metric: badge.metric,
...
export function BadgeManagementPanel() {
  ...
  function openEdit(badge: ManagedBadgeDefinition) { setEditing(badge); setForm(formFor(badge)); setShowForm(true); setError(""); }
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      const payload = { ...form, requirement, displayOrder: Number(form.displayOrder) || 0 };
  ...
  return <div className="space-y-5">
    ...
    {loading?<p className="text-sm" style={{color:C.textMuted}}>Loading badges...</p>:<div className="space-y-3">{visible.map(badge=><div key={badge.id} className="rounded-[18px] bg-white p-4 flex flex-col gap-4 sm:flex-row sm:items-center" style={{boxShadow:"0 1px 3px rgba(27,67,50,0.10)"}}>
      <div className="h-14 w-14 rounded-xl flex items-center justify-center overflow-hidden text-2xl flex-shrink-0" style={{backgroundColor:C.muted}}>{badge.imageUrl?<img src={badge.imageUrl} alt="" className="h-full w-full object-contain"/>:badge.icon||"🏅"}</div>
    ...
    {showForm&&<div className="fixed inset-0 z-[1000] bg-black/50 flex items-center justify-center p-4"><div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-[20px] bg-white p-6"><div className="flex items-start justify-between mb-5"><div><h2 className="text-xl" style={{fontFamily:F.display,color:C.jungle}}>{editing?"Edit Badge":"Add Badge"}</h2><p className="text-xs" style={{color:C.textMuted}}>{editing?"The badge key is locked to protect existing awards.":"New badges are evaluated on the next user badge sync."}</p></div><button onClick={closeForm}><X size={18}/></button></div>
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="text-xs font-bold">Icon or emoji<input value={form.icon} onChange={e=>setForm({...form,icon:e.target.value})} maxLength={20} className="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm"/></label>
        ...
        <label className="text-xs font-bold sm:col-span-2">Badge Icon with Image URL (optional)<input type="url" value={form.imageUrl} onChange={e=>setForm({...form,imageUrl:e.target.value})} placeholder="https://..." className="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm"/></label>
