...
import type { MockUser, ActivityLog, Page } from "../lib/types";
import {
  getSubmissions,
  getAnnouncements,
...
export function AccountPage({ user, setUser, onLogout, logs, bookmarks, setPage, users, setUsers }:{
  user:MockUser; setUser:(u:MockUser)=>void; onLogout:()=>void;
  logs:ActivityLog[]; bookmarks:number[]; setPage:(p:Page)=>void;
  ...
  const [mySubs, setMySubs] = useState<LocationSubmission[]>(() =>
    getSubmissions().filter((s) => s.contributorId === user.id)
  );
  const [announcements, setAnnouncements] = useState<UserAnnouncement[]>(() =>
    getAnnouncements().filter((a) => a.userId === user.id)
  );
  // refresh when opening tabs
  function refreshMine() {
    setMySubs(getSubmissions().filter((s) => s.contributorId === user.id));
    setAnnouncements(getAnnouncements().filter((a) => a.userId === user.id));
  }
  ...
  function saveProfile() {
    ...
    if (!dName.trim() || !dUsername.trim()) {
      setProfileAlert({type:"error",msg:"Name and username are required."}); return;
    }
    if (dUsername.length < 3) {
      setProfileAlert({type:"error",msg:"Username must be at least 3 characters."}); return;
    }
    const updated = {...user, displayName:dName.trim(), username:dUsername.trim(), bio:dBio.trim()};
    setUser(updated);
  ...
  function changePassword() {
    ...
    if (!curPass || !newPass || !confPass) {
      setPassAlert({type:"error",msg:"All password fields are required."}); return;
    }
    if (curPass !== user.password) {
      setPassAlert({type:"error",msg:"Current password is incorrect."}); return;
    }
    if (newPass.length < 6) {
      setPassAlert({type:"error",msg:"New password must be at least 6 characters."}); return;
    }
    if (newPass !== confPass) {
      setPassAlert({type:"error",msg:"New passwords do not match."}); return;
    }
          ...
          <div className="space-y-3">
            ...
            {mySubs.length===0 ? (
              ...
              <div key={s.id} className="bg-white rounded-[18px] p-4 flex gap-3" style={{boxShadow:`0 1px 3px rgba(27,67,50,0.08)`}}>
                {s.photoData ? (
                  <img src={s.photoData} alt="" className="w-20 h-20 rounded-xl object-cover flex-shrink-0" />
                ) : (
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between mb-2">
              ...
              {announcements.some(a=>!a.read) && (
                <button type="button" onClick={()=>{ markAllReadForUser(user.id); refreshMine(); }} className="text-xs font-bold" style={{color:C.forest,fontFamily:F.body}}>Mark all read</button>
              )}
            </div>
            {announcements.length===0 ? (
              ...
              <button
                key={a.id}
                type="button"
                onClick={()=>{ markAnnouncementRead(a.id); refreshMine(); }}
