// FILE PRIMARY OWNER: FONG XIN TONG | Community Leaderboard & Ranking Module
...
export function LeaderboardPage() {
  ...
  useEffect(()=>{
    Promise.all([
      firebaseClient.entities.User.list(),
      firebaseClient.entities.Badge.list("-created_date", 50),
    ]).then(([u,b])=>{setUsers(u);setBadges(b);}).catch(()=>{setUsers([]);setBadges([]);});
  },[]);

  const sorted = useMemo(()=>users
    .filter(u=>u.role!=="admin")
    .map(u=>({
      name:u.full_name||u.username||u.email||"Explorer",
      km:Number(u.total_km||u.totalKm||0),
      checkins:Number(u.checkins||0),
      states:Number(u.states||0),
    }))
    .sort((a,b)=>b[sortBy]-a[sortBy])
    .map((u,i)=>({...u,rank:i+1,medal:i===0?"🥇":i===1?"🥈":i===2?"🥉":""})),[users,sortBy]);
    ...
    <div className="pt-14 min-h-screen" style={{backgroundColor:C.cream}}>
      <div className="max-w-2xl mx-auto px-5 py-8">
        ...
        <div className="bg-white rounded-[18px] overflow-hidden mb-8" style={{boxShadow:`0 1px 3px rgba(27,67,50,0.10), 0 4px 12px rgba(27,67,50,0.06)`}}>
          {sorted.length===0 && <p className="text-sm text-center py-10" style={{color:C.textMuted,fontFamily:F.body}}>No Firebase leaderboard data yet.</p>}
          {sorted.map((u,i)=>(
