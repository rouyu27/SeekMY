...
export default function Profile() {
  ...
  const handleSaveProfile = async (e) => {
    ...
    setMsg(null);
    try {
      const users = JSON.parse(localStorage.getItem("seekmy_User") || "[]");
      const idx = users.findIndex((u) => u.id === user.id);
      if (idx >= 0) {
        users[idx].full_name = fullName;
        localStorage.setItem("seekmy_User", JSON.stringify(users));
        const token = btoa(JSON.stringify({ ...users[idx], password: undefined }));
        // re-login with updated user
        const { password, ...safe } = users[idx];
        localStorage.setItem("seekmy_token", btoa(JSON.stringify(safe)));
        await checkUserAuth();
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      const users = JSON.parse(localStorage.getItem("seekmy_User") || "[]").filter((u) => u.id !== user.id);
      localStorage.setItem("seekmy_User", JSON.stringify(users));
      logout(true);
    } catch (err) {
      setMsg({ type: "error", text: err.message });
