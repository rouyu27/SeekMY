// FILE PRIMARY OWNER: WILSON CHOONG WEI SHAN | Account Authentication UI
...
function firebaseProfileToMockUser(profile: any, fallbackEmail = ""): MockUser {
...
export function AuthModal({ onClose, onLogin }:{
  ...
  async function handleSignIn() {
    ...
    if (firebaseConfigured) {
      try {
        ...
        const loggedIn = firebaseProfileToMockUser(profile, siEmail);
  ...
  async function handleGoogleSignIn() {
    ...
    try {
      ...
      const loggedIn = firebaseProfileToMockUser(profile, profile?.email || "");
      onLogin(loggedIn, loggedIn.role === "admin");
      onClose();
    } catch (error: any) {
  ...
  async function handleRegister() {
    ...
    if (firebaseConfigured) {
      try {
        ...
        const newUser = firebaseProfileToMockUser({ ...profile, username: rUsername }, rEmail);
          ...
          {tab==="signin" && (
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-3 py-1">
                ...
                <span className="text-[11px] font-semibold" style={{color:C.textMuted,fontFamily:F.body}}>OR</span>
                <div className="h-px flex-1" style={{backgroundColor:C.border}} />
              </div>
