...
import type { Page, MockUser } from "../lib/types";
...
export function NavBar({ page, setPage, mobileOpen, setMobileOpen, user, onAuthClick }:{
  ...
  user:MockUser|null; onAuthClick:()=>void;
}) {
  const [moreOpen, setMoreOpen] = useState(false);
  const moreInPage = NAV_MORE.some(n=>n.page===page);

  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white" style={{boxShadow:`0 1px 0 ${C.border}, 0 2px 8px rgba(27,67,50,0.06)`}}>
      <div className="max-w-6xl mx-auto px-5 h-14 flex items-center gap-3">
        ...
        <div className="hidden md:flex items-center gap-0.5">
          ...
          <div className="relative">
            ...
            {moreOpen && (
              <div className="absolute top-10 left-0 w-52 bg-white rounded-2xl py-2 z-50"
      ...
      {mobileOpen && (
        <div className="absolute top-14 left-0 right-0 bg-white z-50 border-t" style={{borderColor:C.border,boxShadow:`0 12px 40px rgba(27,67,50,0.18)`}}>
