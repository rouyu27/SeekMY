...
export function AdminPage({ users: parentUsers, setUsers: setParentUsers, locations: parentLocations, onLogout }:{
  ...
  function renderActivityLogCard(log:ActivityLog){
    const status=log.status||"approved";
    const expanded=expandedActivityId===String(log.id);
    const submitter=log.userName||log.created_by||log.userId||"Unknown user";
    return (
      <div key={String(log.id)} className={card} style={{borderLeft:`4px solid ${status==="pending"?C.amber:status==="rejected"?C.error:C.success}`}}>
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start">
          {log.photoUrl?<button type="button" onClick={()=>window.open(log.photoUrl,"_blank","noopener,noreferrer")} className="h-24 w-24 overflow-hidden rounded-xl border bg-white flex-shrink-0" style={{borderColor:C.border}} aria-label={`Open activity photo for ${log.location}`}><img src={log.photoUrl} alt={`Activity photo for ${log.location}`} className="h-full w-full object-cover"/></button>:<div className="h-24 w-24 flex-shrink-0 rounded-xl border flex items-center justify-center text-[11px]" style={{borderColor:C.border,backgroundColor:C.muted,color:C.error}}>No photo</div>}
          <div className="min-w-0 flex-1">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <p className="font-bold text-sm truncate">{log.location}</p>
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full capitalize" style={{backgroundColor:status==="pending"?"#fffbef":status==="rejected"?C.errorBg:C.successBg,color:status==="pending"?"#92400e":status==="rejected"?C.error:C.success}}>{status}</span>
                </div>
                <p className="text-xs mt-1" style={{color:C.textMuted}}>{submitter} · {log.activity} · {log.state} · {log.date}</p>
                <p className="text-xs mt-1" style={{color:C.textSub}}>{log.distance} km · {log.duration || "No duration"}</p>
              </div>
              <div className="flex flex-wrap gap-2 flex-shrink-0">
                <button type="button" onClick={()=>setExpandedActivityId(expanded?null:String(log.id))} className="h-10 px-3 rounded-lg border text-xs font-bold" style={{borderColor:C.border,color:C.forest,fontFamily:F.body}}>{expanded?"Hide details":"View details"}</button>
                {status==="pending"&&<>
                  <button type="button" onClick={()=>activityAction(log,"approve")} disabled={saving} className="h-10 w-10 rounded-lg flex items-center justify-center disabled:opacity-50" style={{backgroundColor:C.successBg,color:C.success}} aria-label={`Approve activity at ${log.location}`}><CheckCircle size={16}/></button>
                  <button type="button" onClick={()=>activityAction(log,"reject")} disabled={saving} className="h-10 w-10 rounded-lg flex items-center justify-center disabled:opacity-50" style={{backgroundColor:C.errorBg,color:C.error}} aria-label={`Reject activity at ${log.location}`}><X size={16}/></button>
                </>}
              </div>
            </div>
            {(log.notes||log.comment)&&!expanded&&<div className="mt-3 space-y-1">{log.notes&&<p className="text-xs" style={{color:C.textSub}}><strong>Notes:</strong> {log.notes}</p>}{log.comment&&<p className="text-xs" style={{color:C.textSub}}><strong>Place comment:</strong> {log.comment}</p>}</div>}
            {expanded&&<div className="mt-4 rounded-xl border p-4 space-y-4" style={{borderColor:C.border,backgroundColor:C.muted}}>
              {log.photoUrl&&<button type="button" onClick={()=>window.open(log.photoUrl,"_blank","noopener,noreferrer")} className="w-full overflow-hidden rounded-xl border bg-white" style={{borderColor:C.border}} aria-label={`Open full activity photo for ${log.location}`}><img src={log.photoUrl} alt={`Activity photo for ${log.location}`} className="max-h-96 w-full object-contain"/></button>}
              <div className="grid gap-3 md:grid-cols-2">
                <div><p className="text-[11px] font-bold" style={{color:C.textMuted}}>Submitted by</p><p className="text-sm mt-1" style={{color:C.text}}>{submitter}</p></div>
                <div><p className="text-[11px] font-bold" style={{color:C.textMuted}}>Status</p><p className="text-sm mt-1 capitalize" style={{color:C.text}}>{status}</p></div>
                <div><p className="text-[11px] font-bold" style={{color:C.textMuted}}>Location</p><p className="text-sm mt-1" style={{color:C.text}}>{log.location}</p></div>
                <div><p className="text-[11px] font-bold" style={{color:C.textMuted}}>Activity</p><p className="text-sm mt-1" style={{color:C.text}}>{log.activity}</p></div>
                <div><p className="text-[11px] font-bold" style={{color:C.textMuted}}>State</p><p className="text-sm mt-1" style={{color:C.text}}>{log.state||"Not provided"}</p></div>
                <div><p className="text-[11px] font-bold" style={{color:C.textMuted}}>Activity date</p><p className="text-sm mt-1" style={{color:C.text}}>{log.date}</p></div>
                <div><p className="text-[11px] font-bold" style={{color:C.textMuted}}>Distance</p><p className="text-sm mt-1" style={{color:C.text}}>{log.distance} km</p></div>
                <div><p className="text-[11px] font-bold" style={{color:C.textMuted}}>Duration</p><p className="text-sm mt-1" style={{color:C.text}}>{log.duration||"Not provided"}</p></div>
              </div>
              <div><p className="text-[11px] font-bold" style={{color:C.textMuted}}>Notes</p><p className="text-sm mt-1 whitespace-pre-line" style={{color:C.textSub}}>{log.notes||"Not provided"}</p></div>
              <div><p className="text-[11px] font-bold" style={{color:C.textMuted}}>Place comment</p><p className="text-sm mt-1 whitespace-pre-line" style={{color:C.textSub}}>{log.comment||"Not provided"}</p></div>
            </div>}
            {status==="rejected"&&log.rejectionReason&&<p className="text-xs mt-2" style={{color:C.error}}><strong>Admin feedback:</strong> {log.rejectionReason}</p>}
          </div>
        </div>
      </div>
    );
