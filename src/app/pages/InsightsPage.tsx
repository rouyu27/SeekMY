// FILE PRIMARY OWNER: FONG XIN TONG | Personal Stats Dashboard
...
export function InsightsPage({
  ...
  const totalKm = approvedLogs.reduce((sum, log) => sum + Number(log.distance || 0), 0);
  const states = new Set(approvedLogs.map(l => l.state).filter(Boolean)).size;
  const activities = Object.entries(approvedLogs.reduce<Record<string, number>>((acc, l) => {
    acc[l.activity] = (acc[l.activity] || 0) + 1;
    return acc;
  }, {})).sort((a,b)=>b[1]-a[1]);
  const stateCounts = Object.entries(approvedLogs.reduce<Record<string, number>>((acc, l) => {
  ...
  const stats = [
    ...
    { icon:<Activity size={18}/>, label:"Activities", value:String(approvedLogs.length) },
