...
export function HomePage({ setPage, setSelectedLocation, setSelectedState, bookmarks, onBookmark, locations, language = "en" }:{
          ...
          <div className="max-w-lg mx-auto mb-8">
            <div className="flex items-center bg-white rounded-full" style={{boxShadow:"0 4px 24px rgba(0,0,0,0.24)",height:46}}>
              <div className="flex items-center gap-2.5 px-5 flex-1">
                ...
                <input value={query} onChange={e=>setQuery(e.target.value)} placeholder={t(language, "searchPlaceholder")} className="flex-1 outline-none text-sm bg-transparent" style={{color:C.text,fontFamily:F.body}}/>
      ...
      {searchResults && (
        <div className="max-w-5xl mx-auto px-5 py-8">
          ...
          {searchResults.locs.length>0 && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">{searchResults.locs.map(loc=><LocationCard key={loc.id} loc={loc} language={language} bookmarked={bookmarks.includes(loc.id)} onBookmark={()=>onBookmark(loc.id)} onView={()=>{setSelectedLocation(loc);setPage("location");}}/>)}</div></>
          )}
        ...
        <div className="max-w-5xl mx-auto px-5 py-10 space-y-12">
          ...
          <div>
            <div className="flex items-center gap-2.5 mb-5">
              ...
              <h2 className="text-2xl font-normal" style={{fontFamily:F.display,color:C.text}}>{t(language, "hiddenGemSpotlight")}</h2>
            </div>
            <div
              role="button"
              tabIndex={0}
              ...
              <div className="p-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div className="flex items-start gap-4">
                    <div className="w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0" style={{backgroundColor:"rgba(255,255,255,0.20)"}}>
                      <Sparkles size={22} className="text-white"/>
                    </div>
                    <div>
                      <h3 className="text-lg font-normal text-white mb-0.5" style={{fontFamily:F.display}}>{gem?.name || "No hidden gem selected"}</h3>
                      <p className="text-xs mb-3" style={{color:"rgba(255,255,255,0.72)",fontFamily:F.body}}>{gem?.state ? `${gem.state}, Malaysia` : "Choose hidden gems in Admin Location Management"}</p>
                      <p className="text-sm leading-relaxed" style={{color:"rgba(255,255,255,0.78)",fontFamily:F.body,maxWidth:520}}>{(gem as any)?.description || "Ask an admin to mark Firebase locations as hidden gems."}</p>
                      {gem && <p className="text-xs mt-3 font-bold" style={{color:"rgba(255,255,255,0.9)",fontFamily:F.body}}>Tap to view details</p>}
                    </div>
                  </div>
                  <span className="self-start flex-shrink-0 text-[11px] font-bold px-3 py-1 rounded-full" style={{backgroundColor:"rgba(255,255,255,0.20)",color:"#fff",border:"1px solid rgba(255,255,255,0.32)",fontFamily:F.body}}>Hidden Gem</span>
                </div>
              </div>
              {hiddenGems.length > 1 && (
                <div className="flex items-center gap-1.5 px-6 pb-5" onClick={e=>e.stopPropagation()}>
                  {hiddenGems.map((item,i)=>(
                    <button
                      key={item.id}
                      type="button"
                      onClick={e=>{e.stopPropagation();setGemIndex(i);}}
                      aria-label={`Show hidden gem ${item.name}`}
                      className="rounded-full transition-all duration-200"
                      style={{width:gemIndex===i?22:7,height:7,backgroundColor:gemIndex===i?"rgba(255,255,255,0.92)":"rgba(255,255,255,0.36)"}}
                    />
                  ))}
                </div>
              )}
          ...
          <div>
            <div className="flex items-end justify-between mb-5">
              <SectionHead title={t(language, "featuredLocations")}/>
              <button onClick={()=>{setSelectedState("");setPage("explore");}} className="flex items-center gap-1 text-sm font-bold mb-5" style={{color:C.forest,fontFamily:F.body}}>{t(language, "viewAll")} <ChevronRight size={14}/></button>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {locations.filter(l => activeAct==="all" || l.activity===activeAct).slice(0,3).map(loc=><LocationCard key={loc.id} loc={loc} language={language} bookmarked={bookmarks.includes(loc.id)} onBookmark={()=>onBookmark(loc.id)} onView={()=>{setSelectedLocation(loc);setPage("location");}}/>)}
            </div>
          </div>
          {/* State sections */}
          <div><SectionHead title={t(language, "peninsularMalaysia")}/><div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">{PENINSULAR.map(s=><StateFlagCard key={s.code} code={s.code} name={s.name} region="Peninsular Malaysia" onClick={()=>goState(s.code)}/>)}</div></div>
          <div><SectionHead title={t(language, "eastMalaysia")}/><div className="grid grid-cols-2 gap-4">{EAST.map(s=><StateFlagCard key={s.code} code={s.code} name={s.name} region="East Malaysia" onClick={()=>goState(s.code)} large/>)}</div></div>
          <div><SectionHead title={t(language, "federalTerritories")}/><div className="grid grid-cols-3 gap-3">{FED.map(s=><StateFlagCard key={s.code} code={s.code} name={s.name} region="Federal Territory" onClick={()=>goState(s.code)}/>)}</div></div>
          {/* AI CTA */}
