...
export default function App() {
  ...
  useEffect(() => {
    ...
    firebaseClient.auth.me().then(async (profile: any) => {
      ...
      setActivityLogs(logRows.map((l:any)=>({
        ...
        // FongXinTong: carry admin-review fields through so LogPage/AdminPage see them
        status:l.status||"approved", verifiedDistanceKm:l.verifiedDistanceKm, verifiedAccuracyM:l.verifiedAccuracyM, rejectionReason:l.rejectionReason,
        created_by:l.created_by, created_by_id:l.created_by_id,
      })));
  ...
  async function loadUserFirebaseData(current: MockUser) {
    ...
    setActivityLogs(logRows.map((l:any)=>({
      ...
      verifiedDistanceKm:l.verifiedDistanceKm,
      verifiedAccuracyM:l.verifiedAccuracyM,
  ...
  function addLog(l: Omit<ActivityLog, "id">) {
    ...
    firebaseClient.entities.ActivityLog.create(l).then((created:any)=>{
      setActivityLogs(prev => {
        ...
        // FongXinTong: only approved logs should count toward badges — a
        // pending (unreviewed) or rejected check-in shouldn't earn anything.
        // evaluateBadges() itself is untouched; just filtering its input.
        const approvedForBadges = next.filter(x => (x.status ?? "approved") === "approved");
        const statuses = evaluateBadges(approvedForBadges, 0, earnedBadgeIds);
